...
- [ ] **YouTube OAuth token** — Can `pass show openclaw/marcus/youtube-token` decrypt successfully? If the token refresh fails, flag it immediately — nothing works without auth.
...
- [ ] **Database connection** — Can we connect to the `openclaw` database as role `marcus`? If `pass show openclaw/marcus/pgpass` fails or the role is broken, flag it.
